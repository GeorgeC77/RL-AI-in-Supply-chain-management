...
import torch
import copy
from utils_two_stage import Reward_adapter_retailer, Reward_adapter_manufacturer, Action_adapter_r, Action_adapter_reverse_r, Action_adapter_m, Action_adapter_reverse_m
...
class echelon(object):
    ...
    def step(self, action):
        ...
        # self.a[self.t] = Action_adapter_reverse_r(action, self.mu, self.rho, self.sigma)
        self.a[self.t] = Action_adapter_reverse_r(action, self.mu)
...
class echelon_m(object):
    ...
    def reset(self, retailer):
        # self.action_space = [self.mu - 3 * self.sigma / (1 - self.rho ** 2), self.mu + 3 * self.sigma / (1 - self.rho ** 2)]
        ...
        self.demand[:self.m - self.l + 1] = self.mu
        self.demand[self.m - self.l + 1:5] = retailer.WIP[0, :]
        self.demand[5:] = retailer.order
    ...
    def step(self, action):  # action: order quantity
        self.act[self.t] = action
        # self.a[self.t] = Action_adapter_reverse_m(action, self.mu, self.rho, self.sigma)
        self.a[self.t] = Action_adapter_reverse_m(action, self.mu)
        ...
        if self.is_train:
            self.r[self.t - 1] = Reward_adapter_manufacturer(self.r[self.t - 1])
