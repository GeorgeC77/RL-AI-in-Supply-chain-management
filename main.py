    ...
    while total_steps < opt.Max_train_steps:
        ...
        while not done:
            ...
            if total_steps % opt.eval_interval == 0:
                ...
                if score > idx:
                    idx = score
                    agent.save(algo_name, EnvName[opt.EnvIdex], "Opt", opt.rho)
                    print("Opt: "+str(int(total_steps / 1000))+" "+str(int(score)))
            total_steps += 1

            '''save model'''
            if total_steps % opt.save_interval == 0:
                agent.save(algo_name, EnvName[opt.EnvIdex], int(total_steps / 1000), opt.rho)
