...
class DQN_agent(object):
    def __init__(self, **kwargs):
        ...
        self.tau = 0.005
        self.replay_buffer = ReplayBuffer(self.state_dim, self.dvc, max_size=int(1e5))
        if self.Duel:
    ...
    def save(self, algo, EnvName, steps, para_rho, para_metric, idx):
        torch.save(self.q_net.state_dict(), "./model/{}_{}_{}_{}_{}_{}.pth".format(algo, EnvName, steps, para_rho, para_metric, idx))

    def load(self, algo, EnvName, steps, para_rho, para_metric, stage):
        path = ''
        ...
        self.q_net.load_state_dict(torch.load(
            "./model/metric_" + str(para_metric) + "/" + path + "{}_{}_{}_{}_{}_{}.pth".format(algo, EnvName, steps,
                                                                                            para_rho,
                                                                                            para_metric, eche)))
        self.q_target.load_state_dict(torch.load(
            "./model/metric_" + str(para_metric) + "/" + path + "{}_{}_{}_{}_{}_{}.pth".format(algo, EnvName, steps,
                                                                                            para_rho,
                                                                                            para_metric, eche)))
...
class ReplayBuffer(object):
    ...
    def add(self, env, step):
        for i in range(step):
            self.s[i + step * self.ptr] = torch.from_numpy(np.concatenate((env.WIP[i,:], [env.NS[i]], env.demand[i: i+6])))
            self.s_next[i + step * self.ptr] = torch.from_numpy(np.concatenate((env.WIP[i + 1,:], [env.NS[i + 1]], env.demand[i + 1: i + 1 + 6])))
