...
from utils import str2bool, evaluate_policy, Action_adapter, Action_adapter_reverse, Reward_adapter_retailer
...
parser.add_argument('--Lead_time', type=int, default=3, help='Lead time')  # Taka lead time, equals Steve lead time+1
...
parser.add_argument('--seq_len', type=int, default=500,
                    help='Sequence length')  # sequence length for a single simulation run
parser.add_argument('--action_dim', type=int, default=1, help='Action dimension')
parser.add_argument('--metric', type=int, default=2,
                    help='idx of evaluation metric')  # 1: only inventory cost; 2: both inventory and capacity cost
parser.add_argument('--m', type=int, default=5,
                    help='length of past demand vector')  # information provided to the algorithm at each time step
parser.add_argument('--col', type=int, default=0,
                    help='whether collaboration exists. 0: No; 1: Yes')  # fixed to 0 for one stage supply chain
...
def main():
    ...
    if opt.Loadmodel: agent.load(algo_name, EnvName[opt.EnvIdex], "Opt", opt.rho, opt.metric, 1,
                                 opt.col)  # load the Opt model for the retailer
